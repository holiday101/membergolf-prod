...
exports.listEventCards = listEventCards;
exports.updateEventCard = updateEventCard;
...
async function listEvents(req, res) {
    ...
    `;
        const [rows] = await pool_1.pool.query(sql, [end, start, courseId]);
        res.json(rows);
    }
    catch (err) {
...
async function listEventCards(req, res) {
    try {
        requireUserId(req);
        const courseId = requireCourseId(req);
        const id = Number(req.params.id);
...
async function updateEventCard(req, res) {
    try {
        requireUserId(req);
        const courseId = requireCourseId(req);
        const eventId = Number(req.params.id);
        const cardId = Number(req.params.cardId);
        if (!Number.isFinite(eventId) || !Number.isFinite(cardId))
            throw new Error("Invalid id");
        ...
        const [result] = await pool_1.pool.execute(`UPDATE eventCard SET ${fields.join(", ")} WHERE card_id = ? AND event_id = ? AND course_id = ?`, values);
...
async function createEventCard(req, res) {
    try {
        requireUserId(req);
        const courseId = requireCourseId(req);
        const eventId = Number(req.params.id);
        ...
        const [result] = await pool_1.pool.execute(`INSERT INTO eventCard
...
async function createEvent(req, res) {
    try {
        requireUserId(req);
        const courseId = requireCourseId(req);
        const { eventname, eventdescription = null, start_dt, end_dt, handicap_yn = 0, nine_id = null, } = req.body ?? {};
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
        const [result] = await pool_1.pool.execute(sql, [
            courseId,
            eventname,
            eventdescription,
            start,
        ...
        const [rows] = await pool_1.pool.query(`SELECT * FROM eventMain WHERE event_id = ? AND course_id = ?`, [
            result.insertId,
            courseId,
        ]);
...
async function updateEvent(req, res) {
    try {
        ...
        if (!Number.isFinite(id))
            throw new Error("Invalid id");
        const { course_id, eventname, eventdescription, start_dt, end_dt, handicap_yn, nine_id, } = req.body ?? {};
        ...
        if (start_dt !== undefined)
            parseISODateTime(start_dt);
        if (end_dt !== undefined)
            parseISODateTime(end_dt);
        const sql = `
      UPDATE eventMain
      SET
    ...
    `;
        ...
        if (result.affectedRows === 0)
            return res.status(404).json({ error: "Not found" });
        const [rows] = await pool_1.pool.query(`SELECT * FROM eventMain WHERE event_id = ? AND course_id = ?`, [id, courseId]);
        res.json(rows[0]);
    }
    catch (err) {
        res.status(err.message === "Unauthorized" ? 401 : err.message === "Forbidden" ? 403 : 400).json({ error: err.message });
...
async function deleteEvent(req, res) {
    try {
        ...
        if (!Number.isFinite(id))
            throw new Error("Invalid id");
        const [result] = await pool_1.pool.execute(`DELETE FROM eventMain WHERE event_id = ? AND course_id = ?`, [
            id,
            courseId,
        ]);
        if (result.affectedRows === 0)
