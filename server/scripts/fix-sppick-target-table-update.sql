-- Fixes spPick: two UPDATE statements select `card_id` from the very table
-- being updated (subEventPayGross / subEventPayNet) inside a `WHERE ... IN
-- (SELECT ...)` subquery. MySQL rejects this with:
--   "You can't specify target table 'subEventPayGross' for update in FROM clause"
-- Fix: wrap each subquery in a derived table (SELECT * FROM (SELECT ...) AS x),
-- which forces MySQL to materialize it, exempting it from the same-table
-- restriction. Behavior is unchanged -- these were previously working queries
-- until MySQL rejected them outright, so "Post Scores" was failing entirely
-- for eventtype_id = 1 ("Stroke Play") sub-events.

DROP PROCEDURE IF EXISTS spPick;
